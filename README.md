...
Ety is inspired from Python's Type Comments, a Hidden GEM(first form of hinting was the type comment (PEP 484/[https://peps.python.org/pep-0526/](526))).
...
The pre-built distribution packages for the editor plugins are available in the repository:

- **VS Code:** Install [`client/ety-client-0.0.1.vsix`](client/ety-client-0.0.1.vsix) via the *Extensions: Install from VSIX...* command.
- **JetBrains (IntelliJ, WebStorm, PyCharm):** Install [`client-jetbrains/build/distributions/ety-jetbrains-0.0.1.zip`](client-jetbrains/build/distributions/ety-jetbrains-0.0.1.zip) via the *Install Plugin from Disk...* action.
- **Neovim:** There is no packaged file. Install directly via your plugin manager (e.g., `lazy.nvim`) pointing to the [`client-neovim`](client-neovim) directory. See the [Neovim client README](client-neovim/README.md) for details.
...
**Why does the type hover/popup appear next to the variable in IntelliJ? Can it be moved?**
The exact placement of the Hover popup is controlled entirely by the IDE (IntelliJ/VS Code), not the language server. `ety` provides the text range and the markdown content, but the IDE decides where to draw the floating window. 

If you are instead seeing inline ghost text (Inlay Hints) like `let count: number = 0`, this is being generated by IntelliJ's built-in JavaScript engine, as `ety` does not currently implement LSP Inlay Hints. You can control or disable these native hints in JetBrains IDEs under **Settings** > **Editor** > **Inlay Hints** > **TypeScript / JavaScript**.
